# Contributing to Flycast Helm Chart
...
helm template flycast chart/

# Install in a test cluster
helm install flycast-test chart/ --dry-run --debug
```
...
- Update `appVersion` in `Chart.yaml` in your PR when pinning the chart to a specific Flycast image
