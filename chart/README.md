...
[![Version: 1.0.3](https://img.shields.io/badge/Version-1.0.3-informational?style=flat-square)](https://github.com/HenriqZimer/flycast-helm-chart)
...
Sega Dreamcast, Naomi, Naomi 2, and Atomiswave emulator, served as a full desktop over the browser via KasmVNC.
...
```bash
git clone https://github.com/HenriqZimer/flycast-helm-chart.git
cd flycast-helm-chart

helm package chart/
helm install flycast ./flycast-1.0.3.tgz
```
...
### Flycast crashes when launching a game (SIGBUS)

If the dashboard/UI works but a game crashes on boot (check the container logs for
`Unhandled SIGBUS`), Flycast's SH4/ARM7 JIT recompiler may need more `/dev/shm` than the small
...
starts when `/dev/shm` is too small); not independently confirmed on Flycast, but worth trying
...
[linuxserver.io GPU Configuration docs](https://docs.linuxserver.io/images/docker-flycast/#gpu-configuration)):
...
If Flycast itself crashes with `Unhandled SIGBUS` in the container logs on boot (i.e. the dashboard works
but launching a game doesn't), see `shmSize` below and `seccompUnconfined` above — try those first, it's
usually not a GPU problem.

### RomM emulator streaming — not available for Flycast
...
third-party Docker Mod that adds an HTTP broker RomM talks to. **No such mod exists for Flycast**:
[github.com/LoneAngelFayt/flycast-romm-integration](https://github.com/LoneAngelFayt/flycast-romm-integration)
...
today — leave it `false` and use Flycast directly through its own desktop (`service.httpsPort`),
...
| `image.repository` | `lscr.io/linuxserver/flycast` | Container image. |
